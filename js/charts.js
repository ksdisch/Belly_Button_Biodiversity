...
d3.json("samples.json").then((data) => {
  ...
  // d4: Create a variable that filters the metadata array for the object with the desired sample number
  var subjectMetaDataArray = data.metadata.filter(sampleObj => sampleObj.id == sample);
  ...
  // d4 Create a variable that holds the first sample in the metadata array.
  var metaDataResult = subjectMetaDataArray[0];
  ...
  // d4: Create a variable that holds the washing frequency.
  ...
  var gaugeData = [{
    ...
    value: washingFreq,
    gauge: {
      axis: {range: [0, 10], visible: true, tickvals: [0, 2, 4, 6, 8, 10]},
      bar: {color: "black"},
      steps: [
        {range: [0, 2], color: "red"},
